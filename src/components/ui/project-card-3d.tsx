...
const ProjectCard3D: React.FC<ProjectCard3DProps> = ({ project }) => {
  ...
  return (
    <div
      className="group perspective w-full h-80 md:h-96 rounded-xl"
      onClick={handleFlip}
      onFocus={handleFlip}
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 backface-hidden rounded-xl">
          <Card className="w-full h-full flex flex-col overflow-hidden bg-card border-border"> {/* Removed glassmorphic */}
            {project.imageUrl && (
              <div className="relative w-full h-1/2">
                ...
                />
                 <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent"></div> {/* Darker gradient for better text visibility */}
              </div>
            )}
            <CardHeader className={cn("flex-grow p-4", project.imageUrl ? "pt-4" : "pt-6")}> {/* Adjusted padding */}
        ...
        <div className="absolute inset-0 rotate-y-180 backface-hidden rounded-xl">
          <Card className="w-full h-full flex flex-col p-4 bg-card border-border"> {/* Adjusted padding & Removed glassmorphic */}
            ...
            <CardFooter className="flex gap-2 pt-4">
              {project.githubUrl && (
                <Button variant="outline" size="sm" asChild className="border-primary/50 text-primary/80 hover:bg-primary/10 hover:text-primary">
